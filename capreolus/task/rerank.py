...
class RerankTask(Task):
    ...
    def rerank_run(self, best_search_run, train_output_path, include_train=False):
        ...
        # TODO: don't let each extractor put all id2tok in memory
        ...
        train_dataset.prepare(
            best_search_run,
            self.benchmark.folds[fold]["train_qids"],
        ...
        dev_dataset.prepare(
            best_search_run,
            self.benchmark.folds[fold]["predict"]["dev"],
            self.benchmark.qrels,
            self.reranker.extractor,
            relevance_level=self.benchmark.relevance_level,
        )
    ...
    def predict(self):
        ...
        docids = set(docid for querydocs in best_search_run.docids() for docid in querydocs)
        self.reranker.extractor.preprocess(
    ...
    def evaluate(self):
        ...
        searcher_runfiles, reranker_runfiles = self.find_crossvalidated_results()

        if fold not in reranker_runfiles:
