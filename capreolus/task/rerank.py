...
class RerankTask(Task):
    ...
    def filter(self, runs):
        if not self.config["filter"]:
            return runs

        filtered = {}
        for qid in runs:
            if qid not in self.benchmark.ignore_qrels:
                filtered[qid] = runs[qid]
                continue
            for docid, label in runs[qid].items():
                if docid in self.benchmark.ignore_qrels[qid]:
                    continue
                filtered[qid][docid] = docid
        return filtered
    ...
    def rerank_run(self, best_search_run, train_output_path, include_train=False):
        ...
        dev_run = defaultdict(dict)  # This is possible because best_search_run is an OrderedDict
        ...
        # qrel_fn = self.benchmark.qrel_prev_runs if hasattr(self.benchmark, "qrel_prev_runs") else self.benchmark.qrels
        qrel_fn = self.benchmark.prev_qrels if hasattr(self.benchmark, "qrel_ignore") else self.benchmark.qrels
        train_dataset = self.sampler
        train_dataset.prepare(
            train_run, qrel_fn, self.reranker.extractor, relevance_level=self.benchmark.relevance_level,
        )
        dev_dataset = PredSampler()
        dev_dataset.prepare(
            dev_run, qrel_fn, self.reranker.extractor, relevance_level=self.benchmark.relevance_level,
        )
        ...
        self.reranker.trainer.generate_train_predict_tfrecord(self.reranker, train_dataset, dev_dataset, test_dataset)
