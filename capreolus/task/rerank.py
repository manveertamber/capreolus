...
class RerankTask(Task):
    ...
    def rerank_run(self, best_search_run, train_output_path, include_train=False):
        ...
        logger.info(f"docid: {sys.getsizeof(docids)}")
        ...
        train_qrels = {qid: self.benchmark.qrels[qid] for qid in self.folds[fold]["train_qids"]}
        ...
        train_dataset.prepare(
            best_search_run, train_qrels, self.reranker.extractor, relevance_level=self.benchmark.relevance_level
        )
        dev_dataset = PredSampler()
        dev_dataset.prepare(
            dev_run, dev_qrels, self.reranker.extractor, relevance_level=self.benchmark.relevance_level
        )
