...
class Runs:
    ...
    def __init__(self, runfile, format="trec", buffer=True):
        ...
        # print("preparing qid2pos")
        # t1 = time()
        # self.prepare_qid2pos()
        # print("finished preparing", time() - t1, "size of qid2position", sys.getsizeof(self.qid2position) / 1024, "KB")
    ...
    def keys(self):
        if self.qid2position:  # not multithread safe
            for qid in self.qid2position:
                yield qid
        else:
            print("generating qid2position")
            t1 = time()
            last_qid = -1
            last_end = 0  # the cursor position after last read()
            f = self._open_runfile()
            ...
            print(f"finished generating qid2position: ", time() - t1)
            yield last_qid
    ...
    def items(self):
        ...
        prepare_pos = False if self.qid2position else True # must be called after qid2position is fully prepared
        last_end = 0  # the cursor position after last read()
        ...
        last_qid = -1
        doc2score = {}
        for line in self.readlines(f):
            qid, _, docid, rank, score, _ = line.strip().split()
            if (not prepare_pos) and (qid not in self.qid2position):  # ignore deleted terms
                continue
            ...
            if qid != last_qid:
                ...
                if prepare_pos:
                    self.qid2position[qid] = [last_end, f.tell()]  # new qid
                last_qid = qid
                doc2score = {}
            else:
                if prepare_pos:
