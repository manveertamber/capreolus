...
class HiNT(nn.Module):
    ...
    def matrix_inv(self, A):
        ...
        A1 = torch.randn(self.passagelen * self.batch_size, maxqlen, 100, 3).type(torch.FloatTensor).to(device)
        for i in range(maxqlen):
            for j in range(100):
                A1[:, i, j, :] = A[:, maxqlen - i - 1, 99 - j, :]
        return A1
...
class HiNT_main(nn.Module):
    ...
    def forward(self, query_sentence, query_idf, pos_sentence, neg_sentence):
        self.HiNT1.batch_size = query_sentence.size(0)
