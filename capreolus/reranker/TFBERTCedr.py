...
class TFBERTCedr_Class(tf.keras.layers.Layer):
    ...
    def call(self, x, **kwargs):
        """
        :param x: [bert_input, mask, seg], each has shape (B * n_psg, T), (B * n_psg, T), (B * n_psg, T),
        :param kwargs:
        :return:
        """
    ...
    def convert_input(self, bert_input, mask, seg):
        batch_size = tf.shape(bert_input)[0]
    ...
    def score(self, x, **kwargs):
        posdoc_bert_input, posdoc_mask, posdoc_seg, _, _, _ = x
        posdoc_bert_input, posdoc_mask, posdoc_seg = self.convert_input(posdoc_bert_input, posdoc_mask, posdoc_seg)
        passage_scores = self.call((posdoc_bert_input, posdoc_mask, posdoc_seg), **kwargs)[:, 1]
        return passage_scores
